...
static struct platform_driver barrometer_driver = {
	...
	.remove = __devexit_p(barrometer_remove),
	.driver = {
...
static u16 read_uart_reg_raw(u32 reg_offset){
	...
	if(barrometer.uart2_base){
		ioread16(barrometer.uart2_base + reg_offset);
		printk(KERN_INFO "Read 0x%04x to UART register at offset 0x%08x \n", value, reg_offset);
	} else{
		printk(KERN_ERR "UART Register is not mapped!!\n");
	}
	return value;
...
static int __init init_callback_fn(void){
	...
	return platform_driver_register(&barrometer_driver); //Passing in the barrometer_driver struct
...
static void __exit exit_callback_fn(void) {
	...
	platform_driver_unregister(&barrometer_driver);
}
...
static ssize_t loopback_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	barrometer.loopback[sizeof(barrometer.loopback) -1] = '\0'; //Setting null character	

	return count;
}

// Linking the attribute to the show and store functions
static DEVICE_ATTR(loopback, 0644, loopback_show, loopback_store); //0644 allows Read and Write. 0444 only read.
...
static int barrometer_probe(struct platform_device *pdev){
	...
	retval = device_create_file(&pdev->dev, &dev_attr_loopback); // Allows users to interact with attribute through /sys

	if (retval) {
		printk(KERN_INFO "uart_loop: Failed to create sysfs file! \n");
		uart_deinit();
		return retval;
	}
	...
	printk(KERN_INFO "uart_loop: Driver bounded successfully! \n");
...
static int barrometer_remove(struct platform_device *pdev){
	...
	printk(KERN_INFO "uart_loop: Driver unbounded successfully! \n");
	return 0;
}
